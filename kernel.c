...
void console_func(void) {
    ...
    if (read_line(input, sizeof(input))) {
        if (starts_with(input, "cube on")) {
        ...
        else if (starts_with(input, "unlink ")) {
             if (!fs_delete_hardlink(input + 7)) {
                    print("Unlink failed\n");
             }
        }
        else if (starts_with(input, "hlink ")) {
            char* p = input + 6;
            while (*p == ' ') p++;
            char* target = p;
            while (*p && *p != ' ') p++;
            if (*p) {
                *p = 0;
                p++;
                while (*p == ' ') p++;
                if (vfs_create_hardlink(target, p)) {
                    print("Hardlink created\n");
                }
                else {
                    print("Failed to create hardlink\n");
                }
            }
            else {
                print("Usage: hlink <target> <link>\n");
            }
        }
        ...
        else if (streq(input, "poweroff") || streq(input, "shutdown")) {
            print("Powering off...\n");
            acpi_poweroff();
        }
