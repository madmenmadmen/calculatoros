...
static int ehci_enumerate_port(int port) {
    ...
    usb_msc_bot_reset(dev_addr, 0, ehci_control_transfer);
    ehci_mdelay(50);

    int idx = usb_device_count;
    if (!usb_msc_read_capacity(idx, dev_addr, bulk_in, bulk_out, &sectors, &block_size, ehci_bulk_transfer_dev, ehci_control_transfer)) {
...
int ehci_usb_write(int idx, uint32_t lba, uint8_t count, const void* buf) {
    ...
    return usb_msc_write_10(idx, usb_devices[idx].address,
        ...
        lba, count, buf, ehci_bulk_transfer_dev, ehci_control_transfer);
}
