...
# C source files
C_SOURCES = kernel.c keyboard_buffer.c pic.c pit.c idt.c jit.c fs.c ata.c block.c pci.c ahci.c ehci.c usb_msc.c
...
# Run with only EHCI USB (no SATA, no IDE)
...
run-all: $(OS_ISO) $(DISK_IMG) $(SATA_IMG) $(USB_IMG) $(USB2_IMG)
	$(QEMU) \
		...
		-device hda-duplex,id=hda-codec0,bus=hda.0,cad=0 \
		-monitor vc \
		-serial file:serial.log \
		$(QEMU_FLAGS)
