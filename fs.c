...
static uint8_t ram_disk[32 * 1024 * 1024];
static int ram_disk_initialized = 0;
...
static void ram_disk_init(void) {
    ...
    for (int i = 0; i < sizeof(ram_disk); i++) {
        ram_disk[i] = 0;
    }

    ram_disk[SUPERBLOCK_SECTOR * 512 + 0] = 'M';
    ram_disk[SUPERBLOCK_SECTOR * 512 + 1] = 'F';
    ram_disk[SUPERBLOCK_SECTOR * 512 + 2] = 'S';
    ...
    for (int i = 0; i < MAX_BLOCKS_PER_FILE; i++) root.blocks[i] = -1;

    memcpy(&ram_disk[INODE_START * 512], &root, sizeof(inode_t));

    ram_disk_initialized = 1;
    print("RAM disk initialized (32MB)\n");
}
...
static void ram_read(uint32_t lba, uint8_t count, void* buf) {
    uint32_t offset = lba * 512;
    uint32_t size = count * 512;

    if (offset + size > sizeof(ram_disk)) {
        print("RAM read out of bounds!\n");
        return;
    }

    memcpy(buf, &ram_disk[offset], size);
}

static void ram_write(uint32_t lba, uint8_t count, const void* buf) {
    uint32_t offset = lba * 512;
    uint32_t size = count * 512;

    if (offset + size > sizeof(ram_disk)) {
        print("RAM write out of bounds!\n");
        return;
    }

    memcpy(&ram_disk[offset], buf, size);
}

static void dev_read(block_device_t* dev, uint32_t lba, uint8_t count, void* buf) {
    if (ram_mode) {
        ram_read(lba, count, buf);
    }
    else {
...
static int create_inode(fs_instance_t* fs, const char* name, int parent, int type) {
    ...
    in->link_count = (type == TYPE_FILE) ? 1 : 0;
...
int fs_remove_file(const char* path) {
    ...
    if (fs->inode_cache[id].type == TYPE_FILE) {
        if (fs->inode_cache[id].link_count > 0) {

            fs->inode_cache[id].link_count--;
            fs->inode_cache_dirty[id] = 1;
            save_inode(fs, id);
            return 1;
        }
...
int fs_write_file_len(const char* path, const char* data, int len) {
    const char* rel;
    fs_instance_t* fs = vfs_resolve(path, &rel);
    if (!fs) return 0;
    ...
    load_inode(fs, inode_num);
    if (fs->inode_cache[inode_num].type != TYPE_FILE) return 0;
...
int fs_write_file(const char* path, const char* data) {
    int len = 0;
    while (data[len]) len++;
    return fs_write_file_len(path, data, len);
}
...
void fs_list_dir(const char* path) {
    ...
    for (int i = 0; i < child_count; i++) {
        ...
        else if (fs->inode_cache[child].type == TYPE_HARDLINK) {
            print("HLINK ");
        }
        else {
            print("FILE ");
...
static int resolve_symlink(fs_instance_t* fs, int inode_num, int depth) {
    if (depth > 10) {
        print("Too many levels of links\n");
        return -1;
    }
    ...
    if (fs->inode_cache[inode_num].type == TYPE_HARDLINK) {
        int target_inode = fs->inode_cache[inode_num].target_inode;
        if (target_inode < 0) {
            print("Broken hardlink\n");
            return -1;
        }
        return target_inode;
    }
...
int fs_read_file_range(const char* filename, int offset, void* buf, int size) {
    const char* rel;
    fs_instance_t* fs = vfs_resolve(filename, &rel);
    if (!fs) return -1;
    ...
    int file_size = fs->inode_cache[inode_num].size;
    if (offset >= file_size) return 0;
    if (offset + size > file_size) size = file_size - offset;

    return read_file_data(fs, inode_num, (uint8_t*)buf, offset, size);
}

int fs_create_hardlink(const char* target_path, const char* link_path) {
    const char* rel_target, * rel_link;
    fs_instance_t* fs_target = vfs_resolve(target_path, &rel_target);
    fs_instance_t* fs_link = vfs_resolve(link_path, &rel_link);

    if (!fs_target || fs_target != fs_link) {
        print("Hardlink: target and link must be on same filesystem\n");
        return 0;
    }

    int target_inode = find_inode(fs_target, rel_target);
    if (target_inode < 0) {
        print("Hardlink: target not found\n");
        return 0;
    }

    load_inode(fs_target, target_inode);

    if (fs_target->inode_cache[target_inode].type == TYPE_DIR) {
        print("Hardlink: cannot link to directory\n");
        return 0;
    }

    char link_dir[MAX_PATH], link_base[MAX_FILENAME];
    split_path(rel_link, link_dir, link_base);

    int parent = find_inode(fs_link, link_dir);
    if (parent < 0) return 0;

    int link_id = alloc_inode(fs_link);
    if (link_id < 0) return 0;

    inode_t* link_inode = &fs_link->inode_cache[link_id];
    strcpy(link_inode->name, link_base);
    link_inode->type = TYPE_HARDLINK;
    link_inode->size = 0;
    link_inode->parent = parent;
    link_inode->target_inode = target_inode;
    link_inode->first_child_block = -1;
    link_inode->indirect = -1;
    for (int i = 0; i < MAX_BLOCKS_PER_FILE; i++) link_inode->blocks[i] = -1;

    load_inode(fs_target, target_inode);
    fs_target->inode_cache[target_inode].link_count++;
    fs_target->inode_cache_dirty[target_inode] = 1;
    save_inode(fs_target, target_inode);

    fs_link->inode_cache_dirty[link_id] = 1;
    save_inode(fs_link, link_id);

    dir_add_child(fs_link, parent, link_id);

    return 1;
}

int fs_delete_hardlink(const char* path) {
    const char* rel;
    fs_instance_t* fs = vfs_resolve(path, &rel);
    if (!fs) return 0;
    ...
    if (fs->inode_cache[inode_num].type == TYPE_HARDLINK) {
        int target = fs->inode_cache[inode_num].target_inode;
        load_inode(fs, target);
        fs->inode_cache[target].link_count--;
        fs->inode_cache_dirty[target] = 1;
        save_inode(fs, target);

        int parent = fs->inode_cache[inode_num].parent;
        dir_remove_child(fs, parent, inode_num);
        free_inode(fs, inode_num);

        if (fs->inode_cache[target].link_count == 0) {

            for (int i = 0; i < MAX_BLOCKS_PER_FILE; i++) {
                if (fs->inode_cache[target].blocks[i] != -1) {
                    free_block(fs, fs->inode_cache[target].blocks[i]);
                    fs->inode_cache[target].blocks[i] = -1;
                }
            }
            free_inode(fs, target);
        }
        return 1;
    }

    print("Not a hardlink\n");
    return 0;
}

int vfs_create_hardlink(const char* target, const char* link) {
    return fs_create_hardlink(target, link);
}
...
void fs_init(void) {
    ...
    for (int i = 0; i < part_count; i++) {
        if (streq(partitions[i].name, "sda1")) {
    ...
    if (!found_disk) {
        print("No disk found! Running in RAM mode.\n");
        ram_mode = 1;
        ram_disk_init();

        static block_device_t ram_device;
        ram_device.present = 1;
        ram_device.read = NULL;
        ram_device.write = NULL;

        if (fs_mount(&ram_device, "/")) {
            current_fs = &instances[0];
        }
        else {
            print("RAM mount failed!\n");
            reboot();
        }

        fs_update_devices();
        print("RAM mode active\n");
        return;
    }
    ...
    if (magic[0] == 'M' && magic[1] == 'F' && magic[2] == 'S') {
        ...
        print("No filesystem on /dev/sda1, formatting...\n");
        fs_format(root_dev);
        if (fs_mount(root_dev, "/")) {
            current_fs = &instances[0];
        }
